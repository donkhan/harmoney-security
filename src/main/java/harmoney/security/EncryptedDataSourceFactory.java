...
public class EncryptedDataSourceFactory extends BasicDataSourceFactory {
	...
	private String decrypt(int idx, Reference ref) throws Exception {
		CipherModule cm = new CipherModule();
		String decryptedPassword = cm.decrypt(ref.get(idx).getContent().toString());
		//String decryptedPassword = new WybisCipherModule(ENC_KEY).decrypt(ref.get(idx).getContent().toString());
